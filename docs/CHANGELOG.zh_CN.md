...
- 首次发布 Windows x64 单文件安装助手预览版：导入与预览 ICS/CSV、保存虚构 CSV 模板、选择设备并在本地安装，无需开发环境。
- 内置固件模板、中文字库生成和串口工具，在本地生成个人课表并在写入前校验固件。
- 检查芯片、容量、安全设置和原厂分区兼容性；写入校验通过后才切换启动，随后核对课表指纹并校时。
- 保留兼容的原厂分区，分发 EXE，不分发整包覆盖刷机的 full.bin。
- 新模板的真机显示与物理安装流程尚未验收，因此本次标记为预览版。
